...
# -*- coding: utf-8 -*-
"""
号外NET 東近江市・近江八幡市 の公式RSSを取得し、
タイトルに「近江八幡」を含む記事だけを rss_items.json / rss.xml にマージするスクリプト。

このサイトは東近江市・近江八幡市の記事が混在しているため、
タイトルの地域タグ(【近江八幡市】等)で絞り込む。
"""
...
SOURCE_RSS_URL = "https://higashiomi-omihachiman.goguynet.jp/feed/"
TITLE_FILTER_KEYWORD = "近江八幡"  # このキーワードを含むタイトルだけ採用
...
<rss version="2.0">
  <channel>
    ...
    <description>近江八幡市公式サイト・近江八幡経済新聞・号外NETの新着をまとめた非公式RSSです。</description>
...
def main():
    ...
    for item in channel.findall("item"):
        ...
        if TITLE_FILTER_KEYWORD not in title:
    ...
    print(f"号外NET: 新着(近江八幡関連) {len(new_items)}件 / 対象外(東近江市等) {skipped_other_area}件")
