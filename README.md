...
| 出典 | 方式 |
|---|---|
| 近江八幡市公式サイト | ハブページ監視(全体一覧+部署トップ+小区分ページを巡回し、その配下の個別記事を検出) |
| 近江八幡経済新聞 | 公式RSS取得 |
...
| fetch_list_sources.py | RSSのない一覧ページ型サイトの新着収集(商工会議所・健康ふれあい公園) | 5 |
| build_feed.py | 収集結果からrss.xmlを生成(日付順に整列) | 6 |
...
- hubs.json : 市サイトの巡回対象一覧(小区分ページは巡回中に自動追加される)
- hub_cursor.json : ハブ巡回の続きの位置(1回あたり250件ずつ分割して巡回)
